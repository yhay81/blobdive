...
BlobDive currently has one repository owner and one release-capable maintainer,
...
- the released native binary by running `blobdive --version`.
...
GH_TOKEN="$(gh auth token)" \
  ./scripts/continuity-drill.sh yhay81/blobdive blobdive
```
...
and parser-incident runbooks, or the single-maintainer risk remains disclosed
while this drill plus repository-account and package-registry recovery are
tested and recorded without storing secrets in Git. Until then, continuity
remains open.
