...
BlobDive publishes native archives for Linux x86_64, macOS Apple Silicon and
...
archive="blobdive-${version}-${platform}.tar.gz"
gh release download "$version" --repo yhay81/blobdive \
  --pattern "$archive" --pattern SHA256SUMS
awk -v file="./$archive" \
  '$2 == file { print; found = 1 } END { if (!found) exit 1 }' \
  SHA256SUMS | checksum -c -
gh attestation verify "$archive" --repo yhay81/blobdive
tar -xzf "$archive"
install -d "$HOME/.local/bin"
install -m 0755 "${archive%.tar.gz}/blobdive" "$HOME/.local/bin/blobdive"
"$HOME/.local/bin/blobdive" --version
```
...
```powershell
$version = "v0.3.0"
$archive = "blobdive-$version-windows-x86_64.zip"
gh release download $version --repo yhay81/blobdive `
  --pattern $archive --pattern "SHA256SUMS"
...
Copy-Item "blobdive-$version-windows-x86_64\blobdive.exe" `
  (Join-Path $bin "blobdive.exe") -Force
& (Join-Path $bin "blobdive.exe") --version
```
...
`$HOME/.local/bin/blobdive` on macOS/Linux or
`$HOME\.local\bin\blobdive.exe` on Windows. BlobDive does not create a
