...
ArtiProbe identifies a file, opens its internal structure safely, and returns a common graph of nested entries, metadata, dependencies, signatures, and findings. Agents can inspect only the fields and depth they need.

```bash
artiprobe inspect app.zip --depth 3 --fields type,entries,signatures
artiprobe inspect binary --fields arch,imports,exports
artiprobe inspect report.pdf --fields pages,outline,attachments
artiprobe read ref_01J... --limit 100
```
...
ArtiProbe provides one read-only discovery surface while preserving format-specific detail behind typed references.
